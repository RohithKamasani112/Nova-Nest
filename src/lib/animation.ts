...
// ----- Property gallery image transition (directional crossfade + slide) -----
// Both the outgoing and incoming image stay mounted (no mode="wait"), so the
// swap reads as a smooth crossfade with a gentle directional drift. `dir` of 0
// (first open) skips the slide so it composes cleanly with the shared-element
// morph from the property card.
